...
test('rejects bare codex commands without the // prefix', async () => {
  ...
  const expected = [
    ...
    '  //status  - Show bridge and Codex session state',
    '  //abort  - Abort the current task',
    '  //read <path>  - Read a project file and send it to chat as a file',
    '  //model <model>  - Set the active model for the bound project',
    '  //reload projects  - Reload the projects.json file',
    '  //resume <threadId|last>  - Resume a Codex thread for this chat',
    '  //approvals  - List pending approval requests',
    '  //approve <id>  - Approve a single request',
    '  //approve-all <id>  - Approve the request for the whole chat session',
    ...
    '',
    'Codex commands:',
    '  //app/list  - List supported Codex apps',
...
test('shows //read in help output', async () => {
  ...
  const readLine = lines?.find(l => l?.includes('//read <path>'));
  assert.ok(readLine !== undefined, `Expected //read in help output, got: ${JSON.stringify(lines)}`);
});
