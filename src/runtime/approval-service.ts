...
function buildPendingSummaryLines(request: PendingRequest): string[] {
  return [
    `  ${request.requestId} | ${describeRequestKind(request.kind)} | ${request.projectInstanceId} | ${request.threadId}/${request.turnId}`,
  ];
}
...
function parseApprovalCommand(text: string): { command: ApprovalCommand; requestId: string } | null {
  ...
  if (command !== 'approve' && command !== 'approve-all' && command !== 'deny' && command !== 'approvals') {
    return null;
  }
